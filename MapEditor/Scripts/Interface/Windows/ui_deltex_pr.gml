...
switch cmd6
{
  case ui_win5_b1:
    ...
    {
      var tx_ind;
      tx_ind = API_Listbox_GetSelItem(ui_win5_lb1, tx_next, true);
      if ed_tex_delete(tx_ind + 1)
      {
        API_Listbox_DeleteString(ui_win5_lb1, tx_ind);
      } else {
        API_Dialog_MessageBox(ui_win5,
            'Текстура ' + API_Combobox_GetText(ui_tx_cb, tx_ind + 1) + ' не может быть удалена, поскольку все еще используется.',
            'Ошибка', MB_OK|MB_ICONWARNING);
        tx_next += 1;
      }
    }
